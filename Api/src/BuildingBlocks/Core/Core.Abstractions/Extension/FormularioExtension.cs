...
using Core.Abstractions.Types.Formulario;
using System;
...
namespace Core.Abstractions.Extension
{
    public static class FormularioExtension
    {
        ...
        private static List<IControle> GetControles(Type item, FormularioGrupo formGroup)
        {
            ...
            var propBase = new List<string>() { "TypeMensagenCommand", "TId", "Ativo", "DataCadastro", "DataAtualizacao" };
            var msg = string.Empty;

            var json = string.Empty;

            foreach (var property in item.GetProperties().Where(x => x.Name != "JsonIgnoreAttribute" &&   !propBase.Contains(x.Name)))
            {
        ...
        {
            var validacao = new FormularioValidacao()
            {
